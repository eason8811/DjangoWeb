...
del_words = ['的', '了', '是', '今天', '就', '在', '我', '不', '都', '你', '还', '今年',
             ...
             '（', '）', '(', ')', '!', '！', '.', '。', ',', '，', '?', '？', '\'',
             '\"', '’', '“', '‘', '”', ';', ':', '：', '：', '1', '2', '3', '4', '5',
             '6', '7', '8', '9', '0', '年', '月', '日', '号', '把', '吧', '个', '点',
             '让', '从', '情况', '元', '只', '%']


def fig_wordcloud(code):
    df = pd.read_csv(f'Emotional_Analysis/output/data_{code}.csv', index_col=0)
    all_day = pd.read_csv(f'Emotional_Analysis/output/rating.csv', index_col=0)
